...
namespace Akic.Core
{
    public class pagetest
    {
        ...
        public static string GeneratePagerString(int currentPage, int pageSize, int totalCount)
        {
            ...
            //处理首页
            pagerHtmlString.AppendFormat("<li class='start-page'><a href= 'javascript:void(0)' onclick='LeapToPage({0},{1})'>首页</a></li> ",  1, pageSize);

            //处理上一页:如果当前页不是第一页,就加上上一页
            if (currentPage > 1)
            {
                pagerHtmlString.AppendFormat("<li class='prev-page'><a href='javascript:void(0)' onclick='LeapToPage({0},{1})'>上一页</a></li> ",currentPage - 1, pageSize);
            }
            ...
            for (int i = 0; i < curBarCapacity; i++)  //curBarCapacity=3  i=0 1 2
            {
                ...
                if (j == currentPage)
                {
                    ...
                    if (curBarCapacity == pageBarSize)
                    {
                        ...
                        if (currentPage == start + pageBarSize - 1)
                        {
                            ...
                            {
                                endHtmlString.AppendFormat("<li ><a href='javascript:void(0)' onclick='LeapToPage({0},{1})'>{2}</a></li>", currentPage + 1, pageSize, currentPage + 1);
                            }
                            endHtmlString.Append("<li ><span>...</span></li>");
                            endHtmlString.AppendFormat("<li ><a href='javascript:void(0)' onclick='LeapToPage({0},{1})'>{2}</a></li>", totalPages, pageSize, totalPages);
                        }
                    }
                ...
                {
                    pagerHtmlString.AppendFormat("<li ><a 'javascript:void(0)' onclick='LeapToPage({0},{1})'>{2}</a></li> ", j, pageSize, j);
                }
            }
            ...
            {
                pagerHtmlString.AppendFormat("<li class='next-page'><a href='javascript:void(0)' onclick='LeapToPage({0},{1})'>下一页</a></li> " , currentPage + 1, pageSize);
            }

            //处理末页
            pagerHtmlString.AppendFormat("<li id='end-page'><a href='javascript:void(0)' onclick='LeapToPage({0},{1})'>末页</a></li> " , totalPages, pageSize);
